...
class seg_Trainer(object):
    def __init__(self, opt):
        ...
        self.best_mins_iou = 0.

    def build_dataset(self):
        ####################### dataset #########################

        if self.opt.rand_rot:
            train_transform = T.Compose([utils.RandRotSO3(self.opt.with_norm, self.opt.task, [[360, 0], [360, 1], [360, 2]]),
                                         ...
                                         utils.LRF(self.opt.o3d_normal,
                                                  ...
                                                  axes=self.opt.axes,
                                                  with_norm=self.opt.with_norm, 
                                                  task=self.opt.task,
                                                  dataset=self.opt.dataset),
                                         utils.InitialAttributes(self.opt.init_attr)])
            test_transform = T.Compose([utils.RandRotSO3(self.opt.with_norm, self.opt.task, [[360, 0], [360, 1], [360, 2]]),
                                        utils.LRF(self.opt.o3d_normal,
                                                  self.opt.o3d_normal_orien,
                                                  axes=self.opt.axes,
                                                  with_norm=self.opt.with_norm, 
            ...
            test_transform = T.Compose([
                                        utils.RandRotSO3(self.opt.with_norm, self.opt.task, [[180, 0], [180, 1], [180, 2]]),
                                        utils.LRF(self.opt.o3d_normal,
                                                  self.opt.o3d_normal_orien,
                                                  axes=self.opt.axes,
                                                  with_norm=self.opt.with_norm, 
...
#         print(network)
    ...
    def build_losses(self):
        def cal_loss(pred, gold, smoothing=self.opt.smooth):
            gold = gold.contiguous().view(-1,) #[B*N,]
    ...
    def train_iteration(self):
        pred = self.network(self.data) # [B, N, K] N:npoints, K:num_categories=50
        loss = self.get_loss(pred.view(-1, pred.size(-1)), self.label.view(-1,1).squeeze()) # input: [B*N, 50] [B,N]
        loss.backward()
...
#         print('cat:', self.cat.shape)
    ...
    def train_epoch(self):
        ...
        for data in self.train_loader:
            ...
            self.cat = data.y.cuda()
            self.label = data.seg.cuda()
    ...
    def test_iteration(self):
        ...
        pred = self.network(self.data, False)
        loss = self.get_loss(pred.view(-1, pred.size(-1)), self.label.view(-1,1).squeeze())
        preds = pred.max(dim=2)[1]

        self.meter.collect(preds, self.label.view(-1, 2048), loss, self.cat1.unsqueeze(1))
    ...
    def test_epoch(self):
        ...
        if result['mins_iou'] > self.best_mins_iou and self.opt.training:
            self.best_mins_iou = result['mins_iou']
            print(f'The best mean instance iou is {self.best_mins_iou}')
    ...
    def get_loss(self, pred, label):
        loss = self.criterion(pred, label, False)
        return loss
