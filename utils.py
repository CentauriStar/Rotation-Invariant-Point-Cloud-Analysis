...
class RandomShiver(object):
    def __init__(self, with_norm, task):
...
class LRF(object):
    def __init__(self, o3d_normal=False, orien=False, axes=None, with_norm=False, task='cls', dataset='ModelNetNormal'):
        if axes==None:
            axes = ["normal", "bary"]
        else:
            print(axes)
    ...
    def __call__(self, data):
        ...
        if self.task == 'cls':
            if self.dataset == 'ModelNetNormal' and self.with_norm:
...
class RandRotSO3(object):
    def __init__(self, with_norm, task, rotates):
        rotates_list = []
        self.with_norm = with_norm # bool
        self.task = task # ['cls', 'seg']
        for rot in rotates:
    ...
    def __call__(self, data):
        if self.task == 'cls':
            key = 'LRA'
            if self.with_norm:
                data.pos = torch.stack((data.pos, data[key], data.norm), dim=0)
            else:
